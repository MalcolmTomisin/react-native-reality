...
void PlaneRenderer::InitializeGlContent(AAssetManager* asset_manager) {
  shader_program_ = utils::CreateProgram(kVertexShaderFilename,
  ...
  if (!utils::LoadPngFromAssetManager(GL_TEXTURE_2D, "models/trigrid.png")) {
  ...
  utils::CheckGlError("plane_renderer::InitializeGlContent()");
}
...
void PlaneRenderer::Draw(const glm::mat4& projection_mat,
  ...
  glUseProgram(0);
  glDepthMask(GL_TRUE);
  utils::CheckGlError("plane_renderer::Draw()");
}
