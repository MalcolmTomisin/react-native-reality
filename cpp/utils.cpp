    ...
    namespace utils
    {
        ...
        static GLuint LoadShader(GLenum shader_type, const char *shader_source)
        {
            ...
            if (!compiled)
            {
                ...
                LOGE("arcore::utils::Could not compile shader %d:\n%s\n", shader_type,
                     buf);
                free(buf);
                             ...
                             const std::map<std::string, int> &define_values_map)
        {
            ...
            if (program)
            {
                ...
                CheckGlError("arcore::utils::glAttachShader");
                ...
                if (link_status != GL_TRUE)
                {
                    ...
                    if (buf_length)
                    {
                        ...
                        if (buf)
                        {
                            ...
                            LOGE("arcore::utils::Could not link program:\n%s\n", buf);
                            free(buf);
                        }
                    }
        ...
        bool LoadPngFromAssetManager(int target, const char *path)
        {
            ...
            } jniIds = [env]() -> JNIData
            {
                constexpr char kHelperClassName[] =
                    "com/margelo/nitro/arcore/ARAppSystem";
                ...
                jclass helper_class = utils::FindClass(kHelperClassName);
                if (helper_class)
                {
                ...
                LOGE("arcore::utils::Could not find Java helper class %s",
                     kHelperClassName);
                return {};
            }();
                                          ...
                                          glm::mat4 *out_model_mat)
        {
            ...
            {
                LOGE("utils::GetTransformMatrixFromAnchor model_mat is null.");
                return;
            }
            utils::ScopedArPose pose(ar_session);
