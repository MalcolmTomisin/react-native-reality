...
void ObjRenderer::InitializeGlContent(AAssetManager* asset_manager,
  ...
  if (!utils::LoadPngFromAssetManager(GL_TEXTURE_2D, png_file_name.c_str())) {
  ...
  utils::LoadObjFile(obj_file_name, asset_manager, &vertices_, &normals_, &uvs_,
                    &indices_);

  utils::CheckGlError("obj_renderer::InitializeGlContent()");
}
...
void ObjRenderer::compileAndLoadShaderProgram(AAssetManager* asset_manager) {
  ...
  shader_program_ =
      utils::CreateProgram(kVertexShaderFilename, kFragmentShaderFilename,
...
void ObjRenderer::Draw(const glm::mat4& projection_mat,
  ...
  glUseProgram(0);
  utils::CheckGlError("obj_renderer::Draw()");
}
